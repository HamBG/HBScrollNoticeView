...
typedef void(^ZHWonderfulAction)();

typedef NS_ENUM(NSInteger, ZHMarqueeTapMode) {
    ZHMarqueeTapForMove = 1,
    ZHMarqueeTapForAction = 2
};

@interface ZHNoticeMarquee ()<CAAnimationDelegate>
...
@property (nonatomic,copy  ) ZHWonderfulAction     tapAction;
@property (nonatomic,assign) ZHMarqueeTapMode      tapMode;
@property (nonatomic,assign) ZHMarqueeSpeedLevel   speedLevel;
@property (nonatomic,strong) UIView               *middleView;

@end

@implementation ZHNoticeMarquee
...
- (UILabel *)marqueeLbl{
    if (!_marqueeLbl) {
        self.tapMode = ZHMarqueeTapForMove;
        CGFloat h = self.frame.size.height;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    if (self.tapMode == ZHMarqueeTapForMove) {
        [self stop];
    }
}
